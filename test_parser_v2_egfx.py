...
class TestParsing(unittest.TestCase):
    ...
    def test_parse_unknown_1(self):
        ...
        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_header.Pri, 2)
        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_header.Cmd, Rdp.DynamicVirtualChannels.COMMAND_DATA)
        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_data.ChannelId, 7)

        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.descriptor, Rdp.GraphicsPipelineExtention.DataPackaging.DEBLOCK_SINGLE)
        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.bulkData.header_CompressionType, Rdp.GraphicsPipelineExtention.Compression.PACKET_COMPR_TYPE_RDP8)
        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.bulkData.header_CompressionFlags, {Rdp.GraphicsPipelineExtention.Compression.PACKET_COMPRESSED})

        self.assertEqual(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.bulkData.data.header.cmdId, Rdp.GraphicsPipelineExtention.Commands.RDPGFX_CMDID_CAPSCONFIRM)
        
        self.assertEqual(len(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.bulkData.data.payload), 12)
        self.assertEqual(bytes(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.bulkData.data.payload[:4]), bytes.fromhex("00 06 0a 00"))
        self.assertEqual(bytes(pdu.tpkt.mcs.rdp.channel.dyvc_data.GFX_PDU.bulkData.data.payload[-4:]), bytes.fromhex("00 00 00 00"))
